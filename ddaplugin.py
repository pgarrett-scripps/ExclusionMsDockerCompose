...
class DdaPasefPlugin:
    ...
    def analysis_started(self, analysis_directory, reference_time):
        ...
        try:
            if self._exid is not None:
                available_exclusion_lists = get_exclusion_list_files(self._exclusion_api_ip)
                ...
                if self._exid not in available_exclusion_lists:
                    ...
                    clear_active_exclusion_list(self._exclusion_api_ip)
                    save_active_exclusion_list(self._exclusion_api_ip, self._exid)
                _log.info(f"Loading Exclusion list with ID: {self._exid}")
                load_active_exclusion_list(self._exclusion_api_ip, self._exid)
    ...
    def process_candidates(self, candidates, make_candidate, enable_individual_collision_energies):
        ...
        with self._lock:
            ...
            try:
                exclusion_flags = get_excluded_points(self._exclusion_api_ip,
